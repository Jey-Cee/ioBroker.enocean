...
class handleType1 {
	...
	async main() {
		...
		if (dev !== null) {
			...
			for (const e in eep) {
				//TODO: check for multiple EEPs on device if it is the right one for received package
	...
	async main() {
		switch(this.tType) {
			...
			case UTE: {
				...
				if (obj && (teachinData.request === 2 || teachinData.request === 1)) {
					const type = [0xD4];
			...
			case MSC: {
				...
				if (mfr === '0D') {  //check if manufacturer is Eltako
					if (this.telegram.userData === '00d0ff00000436') {
						...
						this.adapter.log.info('Eltako unknown Tipp-Funk device detected: ' + typeof this.telegram.userData);
					}
				} else {
...
async function createObjects(_this, eep, mfr){
	//TODO: check if device exists, if yes add new data to it
	eep = eep.toUpperCase();
	_this.adapter.log.info(`Create objects for ${eep} from ${mfr}`);
	if(mfr === null){
		mfr = 'EnOcean GmbH';
	}
	...
	if (!obj) {
		...
		newEEP.push(eep);
		await _this.adapter.extendObjectAsync(_this.senderID, {
			native: {
				eep: newEEP
			}
		});
