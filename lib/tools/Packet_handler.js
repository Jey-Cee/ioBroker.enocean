	...
	async main() {
		...
		switch(this.tType) {
			...
			case UTE: {
				...
				const channels = await addLeadingZero( teachinData.channels.toString() );
						...
						if (keys[0] === 'statusfield') {
							...
							if (condition.length !== undefined) {
								for (const con in profile.case[c].condition[s]) {
									...
									if (testResult === true || value === parseInt(check.value) && (condition_res !== false || condition_res === null)) {
										condition_res = true;
									} else {
										condition_res = false;
									}
								...
								const value = ByteArray.from(that.telegram.userData).getValue(bitoffs, bitsize);
								const testResult = jsonLogic.apply(check.value, {'value': value});
								if (testResult === true || value === parseInt(check.value) && condition_res !== false) {
									condition_res = true;
								} else {
									condition_res = false;
								}
