...
async function addEltakoDevice(_this, baseID) {
	...
	switch(eltakoDevices[devTelegram].name) {
		case 'tf61j-230v': {
			_this.adapter.log.info('Eltako TF61J-230V detected');
	...
	if (this.telegram.userData === '00d0ff00000436' || this.telegram.userData === '00d0fe0303010075' || this.telegram.userData === '00d0ff00000415') {
	...
	} else if (this.telegram.userData === '00d0ff0000041a') {
		this.adapter.log.info('Eltako FUD61NPN-230V detected');
		...
		data = type.concat(data, baseID, 0x00);
		await this.sendData(data, optionalData, 0x01); //0x01 = Packet Type ERP1
		await createObjects(this, `TF-13-07`, Enocean_manufacturer[`0x00D`]);
		await createObjects(this, `TF-13-06`, Enocean_manufacturer[`0x00D`]);

	} else {
		this.adapter.log.info('Eltako unknown device detected: ' + this.telegram.userData + ' ID: ' + this.senderID);
	}
}
