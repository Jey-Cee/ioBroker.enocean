...
class handleType10{
	...
	async main() {
		/*console.log(this.esp3packet.data);
		console.log(this.telegram.userData);
		console.log(this.telegram.type.toString(16));
		console.log(this.telegram.senderID);
		console.log(this.telegram.status.toString(16));*/
...
class handleTeachIn{
	...
	async main() {
		if(this.adapter.config.esp2Switch === true){
			this.telegram = new RadioTelegram_ERP2(this.ESP3Packet);
		} else {
		...
		if (rorg === rorgIn) {
			for (const c in profile.case) {
				...
				if (profile.case[c].condition !== undefined) {
					...
					for (const s in profile.case[c].condition) {
						...
						if (keys[0] === 'statusfield') {
							...
							const condition = profile.case[c].condition[s];
							console.log(that.telegram.packet);
							console.log(that.telegram.status);
							if (condition.length !== undefined) {
								for (const con in profile.case[c].condition[s]) {
									const check = profile.case[c].condition[s][con];
									const bitoffs = parseInt(check.bitoffs);
				...
				if (condition_res === true || condition_res === null) {
					console.log(`Test 1 ${that.telegram.userData}`);
					if (profile.case[c].datafield !== undefined) {
						console.log(`Test 2 ${that.telegram.userData.toString('hex')}`);
						for (const x in profile.case[c].datafield) {
							const datafield = profile.case[c].datafield[x];
							console.log(`Test 3 ${that.telegram.userData.toString('hex')}`);
				...
				if(profile.case[c].auto_answer === true) {
					...
					that.adapter.queue.push({'data': data, 'optionaldata': optionalData, 'packettype': 0x01});
