...
class handleType1 {
					...
					for (const c in profile.case) {
						...
						if (profile.case[c].condition !== undefined) {
							...
							for (const s in profile.case[c].condition) {
								const keys = Object.keys(profile.case[c].condition[s]);
						...
						if (condition_res === true || condition_res === null) {
							...
							if (profile.case[c].datafield !== undefined) {
								...
								for (const x in profile.case[c].datafield) {
									...
									if (unit !== null) {
										await this.extendObject(this.telegram.senderID, shortcut, {common: {unit: unit}});
									}
						...
						//Check if device waits for an direct answer
						if(profile.case[c].auto_answer === true) {
							...
							for (const s in parameter) {
								const state = await this.adapter.getStateAsync(`${this.adapter.namespace}.${this.telegram.senderID}.${parameter[s]}`);
								await this.setState(`${this.adapter.namespace}.${this.telegram.senderID}.${parameter[s]}`, {ack: true});
							...
							if(dev.native.broadcast){
								this.telegram.senderID = 'ffffffff';
							}
							const tempId = this.telegram.senderID.toUpperCase().match(/.{1,2}/g);
							let receiverID = [];
							...
							switch (rorg) {
								...
								case 'A5': {
									...
									if (data.length > 4 || data.length < 4) {
										this.adapter.log.warn(`The data length for a 4BS telegram is incorrect. The length is ${data.length}`);
									}
									data = type.concat(data, baseID, 0x00);
									break;
								}
								case 'F6': {
									...
									if (data.length > 1 || data.length < 1) {
										this.adapter.log.warn(`The data length for a RPS telegram is incorrect. The length is ${data.length}`);
									}
								...
								case 'D5': {
									type = [0xD5];
									if (data.length > 1 || data.length < 1) {
										this.adapter.log.warn(`The data length for a 1BS telegram is incorrect. The length is ${data.length}`);
									}
									data = type.concat(data, baseID, 0x00);
									break;
								}
							}
							this.adapter.queue.push({'data': data, 'optionaldata': optionalData, 'packettype': 0x01});
	...
	async convertValue(datafield, value) {
		...
		if(datafield.secondArgument && test === null && datafield.value){
			secondArg = await this.getValue(this.telegram.userData, datafield.secondArgument);
	...
	async main(){
		switch(this.code){
			case '01':
				...
				this.debug('Smart ACK reclaim was not successfull');
				break;
			case '02': {
				//SA_CONFIRM_LEARN
			...
			case '04': {
				...
				switch (wCause){
					...
					case 0x08:
						...
						break;
					case 0x09:
						wakeup = 'Unknown reset source – reset reason couldn’t be detected';
						break;
					case 0x10:
						wakeup = 'UART Wake up';
						break;
	...
	async main() {
		...
		switch(this.tType) {
			...
			case RPS: {
				...
				// check eep and t21/nu status bits stricly against spec (EEP V2.6.7)
				let bCreateObj = false;
				switch (this.info.eep[0]) {
					case 'F6-01-01': { 	// EEP F6-01-xx - Switch/Push Buttons
						bCreateObj = true;
						break;
					}
					case 'F6-02-01': 	// EEP F6-02-xx - Rocker Switch/2 Rocker
					case 'F6-02-02':
					case 'F6-02-03': {
						if (t21 == 1 && nu == 1) {
							bCreateObj = true;
					...
					case 'F6-03-01': 	// EEP F6-03-xx - Rocker Switch/4 Rocker
					case 'F6-03-02': {
						if (t21 == 0 && nu == 1) {
							bCreateObj = true;
						}
					...
					case 'F6-04-01': { 	// EEP F6-04-xx - Position Switch/Home Office Application
						if (t21 == 1 && nu == 1) {
							bCreateObj = true;
						}
						break;
					...
					case 'F6-05-00': 	// EEP F6-05-xx - Detectors
					case 'F6-05-02': {
						bCreateObj = true;
						break;
					}
					...
					case 'F6-10-00': { 	// EEP F6-10-xx - Mechanical handle
						if (t21 == 1 && nu == 0) {
							bCreateObj = true;
						}
						break;
				...
				if (bCreateObj) {
					await createObjects(this, this.info.eep[0], this.info.mfr);
					this.adapter.log.info(`${this.info.mfr}/${this.info.name} with EEP ${this.info.eep[0]} detected and added to devices`);
				} else {
					this.adapter.log.info(`${this.info.mfr}/${this.info.name} with EEP ${this.info.eep[0]} and status bits t21=${t21}, nu=${nu} not yet implemented`);
				}


				this.adapter.setState('gateway.teachin', {val: false, ack: true});
				this.adapter.log.info(`Teach-In: RPS (${this.info.eep[0]}) Telegram detected, you have to add this device manually if it wasn't added right now. The ID is "${this.telegram.senderID}"`);
				break;
			}
			case 'D4':
			case 'UTE':
			case UTE: {
				...
				if (obj && (teachinData.request === 2 || teachinData.request === 1)) { //Teach-out
					...
					await this.sendData(this.adapter, data, optionalData, 0x01); //0x01 = Packet Type ERP1
					await this.sendData(this.adapter, data, optionalData, 0x01);
...
async function addEltakoDevice(_this, baseID, offset) {
	...
	switch(eltakoDevices[devTelegram].name) {
		...
		case 'fd62npn-230v': {
			...
			setTimeout( async () => {
				await _this.sendData(_this.adapter, data, optionalData, 0x01); //0x01 = Packet Type ERP1
				await _this.sendData(_this.adapter, data, optionalData, 0x01); //0x01 = Packet Type ERP1
			}, 5 * 1000);
