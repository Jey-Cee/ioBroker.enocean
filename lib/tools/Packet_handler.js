...
class predefinedDeviceTeachIn {
	...
	async main(){
		...
		this.IDoffset = device.baseIDoffset;
		...
		if(device.EEP.length > 0){
			await this.adapter.extendObjectAsync(this.senderID, {
				'native': {
					eep: device.EEP
				}
			});
...
async function createObjects(_this, eep, mfr){
	...
	if (!obj) {
		...
		if (_this.IDoffset) {
			...
			let offset;
			_this.adapter.log.info(gateway.native.BaseID_offset);
...
async function addEltakoDevice(_this, baseID, offset) {
	...
	const IDdecimal = parseInt('0x' + baseID);
	const offs = (offset !== undefined) ? parseInt(offset) : 0;
	const newIDdecimal = IDdecimal + offs + 1;
	const newID = ByteArray.from( newIDdecimal.toString(16).match(/.{1,2}/g) );
	_this.IDoffset = true;
