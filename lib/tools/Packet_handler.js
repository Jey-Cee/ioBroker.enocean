	...
	async main() {
		switch(this.tType) {
			...
			case UTE: {
				...
				const communication = ByteArray.from(this.telegram.userData).getValue(0, 1);
				const teachInResponse = ByteArray.from(this.telegram.userData).getValue(1, 1);
				const teachInRequest = ByteArray.from(this.telegram.userData).getValue(2, 2);
				const command = ByteArray.from(this.telegram.userData).getValue(4, 4);
				const channels = ByteArray.from(this.telegram.userData).getValue(8, 8);
				const mfr8LSB = await addLeadingZero( ByteArray.from(this.telegram.userData).getValue(16, 8).toString(16) );
				const mfr3MSB = await addLeadingZero( ByteArray.from(this.telegram.userData).getValue(29, 3).toString(16) );
				const TYPE = await addLeadingZero( ByteArray.from(this.telegram.userData).getValue(32, 8).toString(16) );
				const FUNC = await addLeadingZero( ByteArray.from(this.telegram.userData).getValue(40, 8).toString(16) );
				const RORG = await addLeadingZero( ByteArray.from(this.telegram.userData).getValue(48, 8).toString(16) );

				if ( (teachInRequest === 2 || teachInRequest === 0) && teachInResponse === 0 && (communication === 1 || communication === 0) ) { 	//Bidirectional Teach-in/-out request with response
					//TODO: if Teach-in-Request has value 2 check if it is already teached-in, yes? then delete and send teach-out response
					const type = [0xD4];
					...
					await createObjects(this, `${RORG}-${FUNC}-${TYPE}`, `0x0${mfr8LSB}`);
				} else if ( (teachInRequest === 2 || teachInRequest === 0) && teachInResponse === 1 && (communication === 1 || communication === 0) ) { //Bidirectional Teach-in/-out request without response
					await createObjects(this, `${RORG}-${FUNC}-${TYPE}`, `0x0${mfr8LSB}`);
				}

				this.adapter.log.info(`Universal Teach-in: Communication: ${communication}, Teach-In-Response: ${teachInResponse}, Teach-in-Request: ${teachInRequest}, Command: ${command}, Channels: ${channels}, MFR8LSB: ${mfr8LSB.toString(16)}, MFR3MSB: ${mfr3MSB.toString(16)}, RORG: ${RORG.toString(16)}, FUNC: ${FUNC.toString(16)}, TYPE: ${TYPE.toString(16)}`);
				break;
			}
			case OneBS: {
...
async function createObjects(_this, eep, mfr){
	eep = eep.toUpperCase();
	_this.log.info(`Create objects for ${eep} from ${mfr}`);
	if(mfr === null){
		mfr = 'EnOcean GmbH';
	}
