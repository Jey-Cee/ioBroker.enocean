...
class RadioTelegram_ERP2 {
	...
	constructor(packet) {
		...
		this.data = packet.data.slice(1, 4 + packet.dataLength -2).toString('hex');

		//Sender ID, e.g. AABBCCDD
		this.senderID = this.data.slice(8, 16).toString('hex');
	...
	main() {
		switch(this.packet.data[0]) {
			case 0x05:
				this.type = 0xF6;
				//Actual user data in this packet
				this.userData = this.data.slice(0, 2);
				break;
			case 0x06:
				this.type = 0xD5;
				//Actual user data in this packet
				this.userData = this.data.slice(0, 1).toString('hex');
				break;
			case 0x07:
				this.type = 0xA5;
				//Actual user data in this packet
				this.userData = this.data.slice(0, 4).toString('hex');
				break;
			default:
